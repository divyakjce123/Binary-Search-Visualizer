...
class ProBinarySearchApp:
    ...
    def _build_ui(self):
        ...
        self.entry_size = tk.Scale(header, from_=5, to=40, orient=tk.HORIZONTAL, bg=THEME["bg_panel"], fg=THEME["fg_text"], highlightthickness=0)
        ...
        self.btn_run = tk.Button(header, text="▶ START", command=self.init_search, bg=THEME["success"], fg="#1e1e1e", font=("Arial", 11, "bold"), padx=20)
        ...
        self.lbl_top_status = tk.Label(var_frame, text="Ready.", bg=THEME["bg_main"], fg=THEME["fg_text"], font=("Consolas", 16, "bold"))
        ...
        self.btn_prev = tk.Button(timeline_frame, text="⏪ Step Back", command=self.step_back, bg="#444", fg="white", font=THEME["font_main"])
        self.btn_prev.pack(side=tk.LEFT, padx=5)
        
        self.btn_play = tk.Button(timeline_frame, text="⏯ Play/Pause", command=self.toggle_play, bg=THEME["accent"], fg="white", font=THEME["font_bold"])
        self.btn_play.pack(side=tk.LEFT, padx=5)
        
        self.btn_next = tk.Button(timeline_frame, text="Step Fwd ⏩", command=self.step_fwd, bg="#444", fg="white", font=THEME["font_main"])
        ...
        self.txt_code = tk.Text(right_panel, height=18, bg="#1e1e1e", fg=THEME["fg_text"], font=THEME["font_code"], padx=10, pady=10, borderwidth=0)
    ...
    def play_sound(self, sound_type):
        ...
        try:
            delay_sec = self.speed_scale.get()
            duration = int(max(20, min(100, delay_sec * 1000 * 0.2))) 

            if sound_type == "found":
                winsound.Beep(1000, duration)
                winsound.Beep(1500, duration*2)
    ...
    def load_custom(self):
        try:
            ...
            arr = [int(x.strip()) for x in txt.split(',')]
    ...
    def reset_ui(self):
        ...
        self.steps = []
        self.current_step = -1
        self.btn_play.config(text="⏯ Play/Pause")
    ...
    def update_visuals(self):
        if not self.steps or self.current_step < 0: return
        ...
        color = THEME["fg_text"]
        if step.status == "found": color = THEME["success"]
        elif step.status == "not_found": color = THEME["warning"]
        elif "Comparing" in step.msg: color = THEME["highlight"]
        ...
        if self.is_playing:
            if step.status == "found": self.play_sound("found")
            elif "Comparing" in step.msg: self.play_sound("compare")
            elif "Eliminate" in step.msg: self.play_sound("eliminate")
    ...
    def draw_bars(self, step=None):
        ...
        # --- MATH FIX ---
        # 1. Base height = 20px (so small bars are visible)
        base_height = 20
        # 2. Maximum possible bar height = baseline - 130px (leaving top 130px empty for labels)
        max_bar_pixel_height = baseline - 130 
        # 3. Available variable height
